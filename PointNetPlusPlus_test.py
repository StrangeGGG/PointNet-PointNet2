import torch
from PointNetPlusPlus import PointNet2Classification
...
print(device)
pointnet2 = PointNet2Classification()
pointnet2.to(device)

# Load a pre-trained model if it exists
if os.path.exists('save2_10_update2_noSA2.pth'):
    pointnet2.load_state_dict(torch.load('save2_10_update2_noSA2.pth'))
...
plt.savefig('confusion_matrix2_10_update2_noSA2.png', bbox_inches='tight', dpi=300)
plt.close()

report_dict = classification_report(all_labels, all_preds, target_names=list(classes.keys()), output_dict=True, zero_division=0)

# Convert to DataFrame (excluding 'accuracy' and 'avg' rows)
df = pd.DataFrame(report_dict).transpose()
df.to_csv('classification2_report_10_update2_noSA2.csv', index_label='class')